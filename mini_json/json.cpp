...
// -------------------------------------------------------------------------------------
template<>
void load(MiniJson::json j, float& v) {
  assert(j.isLiteral());
  v = (float)atof(j.obj->literal.text);
}

template<>
void load(MiniJson::json j, int& v) {
  assert(j.isLiteral());
  v = (int)atol(j.obj->literal.text);
}

template<>
void load(MiniJson::json j, bool& v) {
  assert(j.isLiteral());
  const MiniJson::JObj* jv = j.obj;
  if (jv->literal.type == MiniJson::JObj::LITERAL_BOOL_FALSE)
    v = false;
  if (jv->literal.type == MiniJson::JObj::LITERAL_BOOL_TRUE)
    v = true;
}

template<>
void load(MiniJson::json j, char const*& v) {
  assert(j.isLiteral());
  if (j.obj->isLiteral() && j.obj->literal.type == MiniJson::JObj::LITERAL_STRING)
